...
from src.model_ablation import InTriCaps_refined, rire, wocn, wos
...
def train(args, new_data, train_data, train_label, train_subgraph, val_data, val_label, val_subgraph):
    if args.ablation_name not in ['rire', 'gre']:
    ...
    if args.ablation_name in ['rire', 'gre', 'wocn', 'wos']:
        routing = args.routing
    elif args.ablation_name == 'wor':
        routing = 1
    ...
    elif args.ablation_name in ['gre', 'wor']:
    ...
    for epoch in range(args.epoch):
        ...
        for iters, batch in enumerate(train_iterator):
            ...
            if (iters+1) % save_iteration == 0:
                ...
                if best_auc < val_auc:
                    best_auc = val_auc
                    print('found best model, save to ./pretrained_model/{}.pkl'.format(args.ablation_name))
                    torch.save(model, './pretrained_model/{}.pkl'.format(args.ablation_name))
...
def evaluate(args, new_data, test_subgraph):
    ...
    if args.train == 'True':
        model = torch.load('./pretrained_model/{}.pkl'.format(args.ablation_name))
    elif args.train == 'False':
        model = torch.load('./best_pretrained_model/{}/ablation/{}.pkl'.format(args.dataset, args.ablation_name))
...
if __name__ == "__main__":
    ...
    args.add_argument('--ablation_name', '-e', type=str, required=True, choices=['wocn', 'wos', 'wor', 'rire', 'gre'])  # 'wocn' means without capsule network; 'wos' without sorting; 'wor' without routing; 'rire' randomly initialize relation embedding; 'gre' glove relation embedding
    ...
    args.add_argument("--exp_name", type=str, default='REIR', help='name of the ablation. choose in [IRER]')
