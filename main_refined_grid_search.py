...
def train(args, new_data, train_data, train_label, train_subgraph, val_data, val_label, val_subgraph, test_subgraph):
        ...
        for epoch in range(args.epoch):
            ...
            if best_auc_current_setting < val_auc:
                ...
                torch.save(model, './pretrained_model/{}_{}_{}_{}_{}_{}_{}_{}_{}_{}.pkl'
                           .format(args.dataset, otm, lr, GCN_dim, GCN_layers, num_mid_caps,
                                   routing, batch_size, pad_edge_num, args.exp_name))
            # print('the best auc_current_setting so far:{}'.format(best_auc_current_setting))
...
def evaluate(args, new_data, test_subgraph, otm, lr, GCN_dim, GCN_layers,
    ...
    model = torch.load('./pretrained_model/{}_{}_{}_{}_{}_{}_{}_{}_{}_{}.pkl'
                       .format(args.dataset, otm, lr, GCN_dim, GCN_layers,
                               num_mid_caps, routing, batch_size, pad_edge_num, args.exp_name))
    model.cuda()
    model.eval()
...
if __name__ == "__main__":
    ...
    '''
    args.add_argument("--num_mid_caps", type=int, default=3)
    args.add_argument("--optimizer", type=str, default='Adam')
    args.add_argument("-l", "--lr", type=float, default=1e-3)
    args.add_argument("--GCN_dim", type=int, default=64, help="the dimension of features in GCN")
    args.add_argument("--activation", type=str, default='tanh', help="the activation function used in GCN")
    args.add_argument("--GCN_layers", type=str, default=3, help="the total iterations of GCN")
    args.add_argument("--routing", type=int, default=3, help="the iterations of dynamic routing(The total times of "
                                                             "updating the routing coefficients)")
    args.add_argument("--pad_edge_num", type=int, default=800, help="the number of edge in the subgraph before CapsNet"
    '''
    ...
    if args.train == 'True':
        train(args, new_data, train_data, train_label, train_subgraph, val_data, val_label, val_subgraph, test_subgraph)
